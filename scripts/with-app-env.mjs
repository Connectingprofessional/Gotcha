#!/usr/bin/env node
/**
 * Run a command with `.dev/app-env.json` merged into its environment.
 *
...
export const APP_ENV_REL_PATH = ".dev/app-env.json";
...
function main(argv) {
  ...
  // Windows needs a shell to resolve npm's `.cmd` shims (vite, tsc, etc.) —
  // `spawn` without it throws ENOENT there even though the binary exists.
