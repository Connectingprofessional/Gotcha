...
const extractCurrency = (value: string) => /([$€£₹]|USD|EUR|GBP|INR|SGD|AED)/i.test(value);
export function scoreOpportunity(job: Job, user: User): MatchBreakdown {
  ...
  const experience = Math.max(45, Math.round(skillScore * .45 + titleScore * .55));
  ...
  const salary = user.salaryGoal && extractCurrency(user.salaryGoal) ? 82 : 76;
...
export function applicationAnalytics(applications: Application[]): ApplicationAnalytics { const total = applications.length; const applied = applications.filter(a => a.status === "applied").length; const interviews = applications.filter(a => a.status === "interview").length; const assessments = applications.filter(a => a.status === "assessment").length; const offers = applications.filter(a => a.status === "offer").length; const rejected = applications.filter(a => a.status === "rejected").length; return { total, applied, interviews, assessments, offers, rejected, responseRate: total ? Math.round(((interviews + offers + rejected) / total) * 100) : 0, interviewRate: total ? Math.round(((interviews + offers) / total) * 100) : 0, offerRate: total ? Math.round((offers / total) * 100) : 0 }; }
export function careerHuntScore(user: User | null, applications: Application[]): number { if (!user) return 0; const profile = Math.min(100, 45 + user.skills.length * 5 + (user.about ? 10 : 0) + (user.targetRoles?.length ? 8 : 0) + (user.targetCountries?.length ? 5 : 0)); const activity = Math.min(100, 50 + applications.length * 2 + applications.filter(a => a.status === "interview").length * 5 + applications.filter(a => a.status === "offer").length * 8); return Math.round(profile * .45 + activity * .55); }
