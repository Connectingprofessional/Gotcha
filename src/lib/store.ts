...
export type ViewId = "dashboard" | "search" | "opportunities" | "applications" | "coach" | "cv" | "hunt" | "market" | "learn" | "saved" | "network" | "profile" | "admin";
...
export type CareerSignal = { id: string; title: string; description: string; priority: "high" | "medium" | "low"; createdAt: string };
...
export const useGotcha = create<State>()(persist((set, get) => ({
  ...
  hydrateFromAuth: (email, name) => {
    ...
    if (!existing) {
      set({ users: [...get().users, { name: name || normalized, email: normalized, password: "", title: "", about: "" } as User] });
    }
    set({ sessionEmail: normalized, view: "dashboard" });
  },
  setView: view => set({ view }), setFilters: f => set({ filters: { ...get().filters, ...f } }),
  applyTo: jobId => { if (get().applications.some(a => a.jobId === jobId)) return; const appliedAt = new Date().toISOString().slice(0, 10); const activeVariant = get().cvVariants.find(v => v.id === get().activeCvVariantId); set({ applications: [{ id: `a-${Date.now()}`, jobId, status: "applied", appliedAt, nextFollowUpAt: computeFollowUpDate(appliedAt), cvVariant: activeVariant?.name }, ...get().applications] }); get().recordCareerEvent("application.created", { jobId, source: "gotcha" }, jobId); },
  ...
  activateCvVariant: id => { const v = get().cvVariants.find(x => x.id === id); if (v) set({ cvText: v.cvText, activeCvVariantId: id }); },
  saveCoverLetter: c => {
  ...
  runCareerAgent: () => {
    const { applications, savedJobIds, cvVariants } = get();
    const signals = deriveCareerSignals({ applications, jobs: JOBS, savedJobIds, hasCvVariant: cvVariants.length > 0 });
    const createdAt = new Date().toISOString();
    set({
      careerSignals: signals.map((s, i) => ({ ...s, id: `signal-${Date.now()}-${i}`, createdAt })),
      lastAgentRunAt: createdAt,
    });
    get().recordCareerEvent("agent.recommendation", { signalCount: signals.length });
  },
}), { name: "gotecha-v2" }));
