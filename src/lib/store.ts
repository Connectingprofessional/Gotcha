...
export const useGotcha = create<State>()(persist((set, get) => ({
  ...
  updateProfile: p => { const email = get().sessionEmail; if (!email) return; set({ users: get().users.map(u => u.email === email ? { ...u, ...p } : u) }); get().recordCareerEvent("profile.updated", { fields: Object.keys(p).join(",") }); }, setDemoOpen: demoOpen => set({ demoOpen }), setAdminPrompt: adminPrompt => set({ adminPrompt }), setAuthPrompt: authPrompt => set({ authPrompt }), setCvText: cvText => set({ cvText }),
  saveCvVariant: v => {
